...
def draw_rect(
    *, pos=0, pos2=None, size=None, align=0, rint=False, color=1, alpha=1, rounding=0
    ...
    pos = np.broadcast_to(np.asarray(pos, dtype="float32"), [2])
    pos2 = (
        np.broadcast_to(np.asarray(pos2, dtype="float32"), [2])
        if pos2 is not None
        else None
    )
    size = (
        np.broadcast_to(np.asarray(size, dtype="float32"), [2])
        if size is not None
        else None
    )
    size = (
        ...
        if pos2 is not None
        else np.array([1, 1], dtype="float32")
    )
    pos = pos - size * align
    if rint:
        pos = np.rint(pos)
    ...
    draw_shape(
        vertices, mode=gl.GL_TRIANGLE_FAN, pos=pos, size=size, color=color, alpha=alpha
    )
