    ...
    def draw_frame(self):
        ...
        if "image" in self.result:
            ...
            if self.drag_widget.show_mask and hasattr(self.drag_widget, "mask"):
                ...
                if self._mask_obj is None or not self._mask_obj.is_compatible(
                    image=self._tex_img
                ):
                ...
                self._mask_obj.draw(
                    pos=pos, zoom=zoom, align=0.5, rint=True, alpha=0.15
                )
            ...
            for point in self.drag_widget.targets:
                ...
                gl_utils.draw_circle(
                    center=np.array([pos_x, pos_y]), color=[0, 0, 1], radius=9 * rescale
                )
            ...
            for point in self.drag_widget.points:
                ...
                pos_y = max_h / 2 + (point[0] - self.image_h // 2) * zoom
                gl_utils.draw_circle(
                    center=np.array([pos_x, pos_y]), color=[1, 0, 0], radius=9 * rescale
                )
            ...
            for point, target in zip(self.drag_widget.points, self.drag_widget.targets):
                ...
                gl_utils.draw_arrow(
                    p_x, p_y, t_x, t_y, l=8 * rescale, width=3 * rescale
                )

            imshow_w = int(self._tex_obj.width * zoom)
            imshow_h = int(self._tex_obj.height * zoom)
            self._image_area = [
                int(self.pane_w + max_w / 2 - imshow_w / 2),
                int(max_h / 2 - imshow_h / 2),
                imshow_w,
                imshow_h,
            ]
