    ...
    def __init__(
        ...
        if use_noise:
            if self.square:
                ...
                self.register_buffer(
                    "noise_const", torch.randn([resolution, resolution // 2])
                )
    ...
    def forward(self, x, w, noise_mode="random", fused_modconv=True, gain=1):
        ...
        if self.use_noise and noise_mode == "random":
            if self.square:
                ...
                noise = (
                    torch.randn(
                        [x.shape[0], 1, self.resolution, self.resolution // 2],
                        device=x.device,
                    )
                    * self.noise_strength
                )
        ...
        **layer_kwargs,  # Arguments for SynthesisLayer.
        ...
        if in_channels == 0:
            if self.square:
                ...
                self.const = torch.nn.Parameter(
                    torch.randn([out_channels, resolution, resolution // 2])
                )
    ...
    def forward(self, x, img, force_fp32=False):
        ...
        if self.in_channels == 0 or self.architecture == "skip":
            if self.square:
                ...
                misc.assert_shape(
                    img,
                    [None, self.img_channels, self.resolution, self.resolution // 2],
                )
