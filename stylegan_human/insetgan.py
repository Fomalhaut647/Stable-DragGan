...
class InsetGAN(torch.nn.Module):
    ...
    def get_body_mask(A, crop, padding=4):
        ...
        mask[
            :,
            :,
            crop[1] - padding : crop[3] + padding,
            crop[0] - padding : crop[2] + padding,
        ] = 0
        return mask
    ...
    def detect_face_dlib(self, img):
        ...
        aligned_image, crop, rect = align_face_for_insetgan(
            img=img,
    ...
    def dual_optimizer(
        ...
        def visual_(
            ...
            step,
            both=False,
            init_body_with_face=None,
        ):
            tmp = synth_body.clone().detach()
            tmp[:, :, body_crop[1] : body_crop[3], body_crop[0] : body_crop[2]] = (
                synth_face
            )
            if both:
        ...
        def forward(
            ...
            if update_crop:
                old_r = (
                    (body_crop[3] - body_crop[1]) // 2,
                    (body_crop[2] - body_crop[0]) // 2,
                )
                _, body_crop, _ = self.detect_face_dlib(synth_body)
                center = (
                    (body_crop[1] + body_crop[3]) // 2,
                    (body_crop[0] + body_crop[2]) // 2,
                )
                body_crop = (
                    center[1] - old_r[1],
                    center[0] - old_r[0],
            ...
            synth_body_face = synth_body[
                :, :, body_crop[1] : body_crop[3], body_crop[0] : body_crop[2]
            ]

            if synth_face.shape[2] > body_crop[3] - body_crop[1]:
                synth_face_resize = F.interpolate(
                    synth_face,
                    size=(body_crop[3] - body_crop[1], body_crop[2] - body_crop[0]),
                    mode="area",
                )
        ...
        _, _, face_crop = self.detect_face_dlib(
            ref_face
        )  # NOTE: this is face rect only. no FFHQ type.
        # create optimizer
        face_optimizer = torch.optim.Adam(
            [face_w_opt, face_w_delta],
            betas=(0.9, 0.999),
            lr=face_initial_learning_rate,
        )
        body_optimizer = torch.optim.Adam(
            [body_w_opt, body_w_delta],
            betas=(0.9, 0.999),
            lr=body_initial_learning_rate,
        )
        ...
        for step in pbar:
            ...
            synth_body, synth_body_face, synth_face_raw, synth_face, body_crop = (
                forward(face_w_opt, body_w_opt, face_w_delta, body_w_delta, body_crop)
            )
        ...
        return (
            ...
            body_w_opt.repeat([1, 18, 1]) + body_w_delta,
            body_crop,
        )
...
@click.option(
    "--face_network",
    default="./pretrained_models/ffhq.pkl",
    help="Network pickle filename",
    required=True,
)
@click.option(
    "--body_network",
    default="./pretrained_models/stylegan2_1024.pkl",
    help="Network pickle filename",
    required=True,
)
...
@click.option(
    "--joint_steps", type=int, default=500, help="num steps for joint optimization"
)
@click.option(
    "--trunc",
    "truncation_psi",
...
def main(
    ...
    face_w = insgan.face_generator.get_latent(
        torch.from_numpy(face_z).to(device)
    )  # [N, L, C]
    ...
    body_w = insgan.body_generator.get_latent(
        torch.from_numpy(body_z).to(device)
    )  # [N, L, C]
