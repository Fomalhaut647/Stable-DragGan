...
def make_latent_interp_animation(
    G,
    code1,
    code2,
    img1,
...
@click.option(
    ...
    default=True,
    type=bool,
    help="select True if you want to save all interpolated images",
)
@click.option("--fps", default=15, help="FPS for GIF", type=int)
@click.option(
    "--num_interps", default=100, help="Number of interpolation images", type=int
)
def main(
    ctx: click.Context,
    network_pkl: str,
    ...
    z1 = torch.from_numpy(np.random.RandomState(seeds[0]).randn(1, G.z_dim)).to(
        device, dtype=dtype
    )
    z2 = torch.from_numpy(np.random.RandomState(seeds[1]).randn(1, G.z_dim)).to(
        device, dtype=dtype
    )
