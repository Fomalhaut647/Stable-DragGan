...
@click.option(
    "--attr_name",
    help="choose one of the attr: upper_length or bottom_length",
    type=str,
    required=True,
)
@click.option(
...
@click.option(
    ...
    type=bool,
    default=True,
    help="If want to combine different editing results in the same frame",
)
...
@click.option(
    "--real_img_path",
    help="Path of real image, this just concat real image with inverted and edited results together",
)
def main(
    ctx: click.Context,
    ckpt_path: str,
    ...
    with torch.no_grad():
        ...
        for t in seeds:
            if real:  # now assume process single real image only
                ...
                test_input = (
                    torch.from_numpy(np.random.RandomState(t).randn(1, 512))
                    .float()
                    .cuda()
                )  # torch.Size([1, 512])
                output, _ = generator(
            ...
            if gen_video:
                ...
                if real:
                    video_ifg_path = (
                        f"{outdir}/video/ifg_{attr_name}_{real_w_path.split('/')[-2]}/"
                    )
                    video_ss_path = (
                        f"{outdir}/video/ss_{attr_name}_{real_w_path.split('/')[-2]}/"
                    )
                    video_sefa_path = (
                        f"{outdir}/video/ss_{attr_name}_{real_w_path.split('/')[-2]}/"
                    )
                ...
                for i in range(total_step):
                    ...
                    if combine:
                        ...
                        legacy.visual(
                            comb_img, os.path.join(video_comb_path, f"{i:05d}.jpg")
                        )
                    else:
                        legacy.visual(
                            image1, os.path.join(video_ifg_path, f"{i:05d}.jpg")
                        )
                        legacy.visual(
                            image2, os.path.join(video_ss_path, f"{i:05d}.jpg")
                        )
                if combine:
