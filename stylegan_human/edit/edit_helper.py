...
def encoder_ifg(
    ...
    if truncation < 1:
        if not real:
            ...
            styles = torch.add(
                truncation_latent,
                torch.mul(torch.sub(noise, truncation_latent), truncation),
            )
    ...
    for i in range(15):
        style_vect = load_pkl(
            os.path.join(latent_dir, "{}/style_vect_mean_{}.pkl".format(attr_name, i))
        )
...
def encoder_ss(
    ...
    style_vect = load_pkl(
        os.path.join(latent_dir, "{}/style_vect_mean_{}.pkl".format(attr_name, layer))
    )
    style_vect = torch.from_numpy(style_vect).to(latent.device).float()
...
def encoder_sefa(
    ...
    sefa_vect = (
        torch.load(os.path.join(latent_dir, "{}.pt".format(attr_name)))
        .to(latent.device)
        .float()
    )
    if step != 0 and total != 0:
