...
def crop_img_with_padding(img, keypoints, rect):
    ...
    if h / w >= 2:  # pad horizontally
        ...
        im_pad = cv2.copyMakeBorder(
            cropped_img, 0, 0, int(pad_left), int(pad_right), cv2.BORDER_REPLICATE
        )
    else:  # pad vertically
        target_h = w * 2
        ...
        cropped_img = img[int(ymin) : int(ymax), int(xmin) : int(xmax)]
        im_pad = cv2.copyMakeBorder(
            cropped_img, int(pad_up), int(pad_down), 0, 0, cv2.BORDER_REPLICATE
        )
...
def run(args):
    ...
    human_seg_args = {}
    human_seg_args["cfg"] = (
        "PP_HumanSeg/export_model/deeplabv3p_resnet50_os8_humanseg_512x512_100k_with_softmax/deploy.yaml"
    )
    ...
    for fname, image in tqdm(dataloader):
        ...
        if (int(extTop[1]) <= 5 and int(extTop[1]) > 0) and (
            comb.shape[0] > int(extBot[1]) and int(extBot[1]) >= comb.shape[0] - 5
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output-folder", dest="output_folder", default="results", type=str
    )
