...
@click.command()
@click.option(
    "--network", "network_pkl", help="Path to network pickle filename", required=True
)
@click.option(
    "--row-seed",
    "src_seed",
    type=legacy.num_range,
    help="Random seed to use for image source row",
    required=True,
)
@click.option(
    "--col-seeds",
    "dst_seeds",
    type=legacy.num_range,
    help="Random seeds to use for image columns (style)",
    required=True,
)
@click.option(
    "--col-styles",
...
@click.option(
    "--only-stylemix",
    "only_stylemix",
    help="Add flag to only show the style mxied images in the video",
    default=False,
)
@click.option(
    "--trunc",
...
@click.option(
    ...
    "duration_sec",
    type=float,
    help="Duration of video (default: %(default)s)",
    default=10,
)
@click.option(
    "--fps",
    "mp4_fps",
    type=int,
    help="FPS of generated video (default: %(default)s)",
    default=10,
)
...
@click.option(
    ...
    default="outputs/stylemixing_video",
    metavar="DIR",
)
def style_mixing_video(
    ...
    src_z = (
        np.random.RandomState(*src_seed).randn(*src_shape).astype(np.float32)
    ...
    dst_z = np.stack(
        [np.random.RandomState(seed).randn(Gs.z_dim) for seed in dst_seeds]
    )
    ...
    src_images = (
        (src_images.permute(0, 2, 3, 1) * 127.5 + 128).clamp(0, 255).to(torch.uint8)
    )

    # Generate the column images:
    dst_images = Gs.synthesis(dst_w, noise_mode=noise_mode)
    dst_images = (
        (dst_images.permute(0, 2, 3, 1) * 127.5 + 128).clamp(0, 255).to(torch.uint8)
    )
    ...
    canvas = PIL.Image.new(
        "RGB",
        ((W - indent_range) * (len(dst_seeds) + 1), H * (len(src_seed) + 1)),
        "white",
    )  # W, H

    for col, dst_image in enumerate(list(dst_images)):  # dst_image:[3,1024,512]
        canvas.paste(
            PIL.Image.fromarray(dst_image.cpu().numpy(), "RGB"),
            ((col + 1) * (W - indent_range), 0),
        )  # H
    ...
    def make_frame(t):
        ...
        canvas.paste(
            PIL.Image.fromarray(src_image.cpu().numpy(), "RGB"), (0 - indent_range, H)
        ...
        for col, dst_image in enumerate(list(dst_images)):
            ...
            col_images = (
                (col_images.permute(0, 2, 3, 1) * 127.5 + 128)
                .clamp(0, 255)
                .to(torch.uint8)
            )
    ...
    videoclip.write_videofile(
        os.path.join(outdir, mp4), fps=mp4_fps, codec=mp4_codec, bitrate=mp4_bitrate
    )
